...
function M.cell_start() return vim.fn.search(cell_delim, "bcWn") end

function M.cell_end()
    local pos = vim.fn.search(cell_delim, "Wn") - 1
    if pos == -1 then pos = vim.api.nvim_buf_line_count(0) end
    return pos
end
...
function M.goto_next_cell_start() vim.fn
    .setpos(".", {0, M.cell_end() + 1, 0, 0}) end
